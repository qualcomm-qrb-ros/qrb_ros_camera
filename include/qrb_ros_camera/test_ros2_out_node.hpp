...
#ifndef TEST_ROS2_OUT_NODE_HPP_
#define TEST_ROS2_OUT_NODE_HPP_
...
{
class TestOutNode : public rclcpp ::Node
{
public:
  explicit TestOutNode(const rclcpp::NodeOptions & options);
  explicit TestOutNode(const std::string & name, const rclcpp::NodeOptions & options);
  int64_t cal_latency(int64_t sec, int64_t nsec);
  int64_t get_cur_time();
  void show_receive_fps(CameraRos2Frame * frame);

private:
  void handler_callback(sensor_msgs::msg::Image::ConstSharedPtr image);
